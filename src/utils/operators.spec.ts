...
describe("NormalizeImage", () => {
  it("normalizes an image correctly hwc", async () => {
    ...
    const imgData = cv.matFromArray(
      1000,
      1000,
      cv.CV_8UC3,
      Array(1000 * 1000 * 3).fill(2),
    );
...
describe("DetResizeForTest", () => {
  ...
  it("throws if input is not cv.Mat", () => {
    ...
    expect(() =>
      op.execute({ image: "not a Mat", shape: null } as any),
    ).toThrow();
  });
});
...
describe("ToCHWImage", () => {
  ...
  it("throws error if input is not Mat", () => {
    const toCHW = new ToCHWImage({ cv });
    expect(() =>
      toCHW.execute({ image: "not a Mat", shape: null } as any),
    ).toThrow();
  });
});
...
describe("KeepKeys", () => {
  it("keeps only specified keys", () => {
    ...
    expect(result).toEqual([shape, array]);
  });
  ...
  it("ignores keys not in data", () => {
    ...
    const keepKeys = new KeepKeys({
      keep_keys: ["shape", "nonexistent"] as any,
    });
    const result = keepKeys.execute(data);
