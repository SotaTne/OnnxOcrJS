...
describe('ndArrayToMat', () => {
  ...
  it('converts 3D uint8 array (RGB image-like) to Mat', () => {
    const arr = ndarray(new Uint8Array([
      255, 0, 0,
      0, 255, 0,
      0, 0, 255,
      255, 255, 0
  ...
  it('throws for >3D array', () => {
    ...
    expect(() => ndArrayToMat(arr, cv)).toThrow(/only 1D, 2D, 3D arrays are supported/);
  });

  it('throws for unsupported channels', () => {
    const arr = ndarray(new Float32Array([1, 2, 3, 4, 5]), [1, 1, 5]);
    expect(() => ndArrayToMat(arr, cv)).toThrow(/only 1,2,3,4 channels are supported/);
  });
